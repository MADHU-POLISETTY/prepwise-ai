...
import { generateFallbackIdealAnswer as getImportedFallbackAnswer } from "./api/_utils";
...
app.post("/api/evaluate-answer", async (req, res) => {
  ...
  // Pre-filter with a robust gibberish / empty check to save latency and avoid unnecessary API calls
  if (!cleanAnswer) {
    return res.json({
      isMeaningful: false,
      isRelevant: false,
  ...
  try {
    const evaluationPrompt = `You are an elite, highly professional technical interviewer and senior talent coach.
...
Your task:
1. Formulate a direct, technically perfect, and comprehensive model answer (ideal response) for the question (maximum 120 words, clean bullet points if helpful).
2. Evaluate the candidate's answer strictly against this ideal model answer:
   - If the candidate's answer is empty, blank, or completely gibberish (e.g., random keyboard mashing like "rtber", "asdf", "bhyh3rtbeg", "skip", "idk"), you MUST score it EXACTLY 0 out of 10 and mark isMeaningful=false, isRelevant=false, isTechnicallyCorrect=false.
   - If the answer is completely off-topic or unrelated to the question, you MUST score it EXACTLY 0 out of 10 and mark isRelevant=false, isTechnicallyCorrect=false.
   - If the answer is extremely brief, superficial, or lacks any technical depth (e.g. mashing buzzwords or typing only a few letters like "rtber"), score it between 0 and 2 out of 10.
   - If the answer shows a basic understanding but is missing key concepts, has moderate conceptual depth, or contains technical inaccuracies, score it between 4 and 6 out of 10.
   - If the answer is solid, technically correct, and covers the main points well, score it between 7 and 8 out of 10.
   - If the answer is exceptional, comprehensive, and demonstrates professional mastery with clear structure, precise terminology, and depth, score it between 9 and 10 out of 10.
3. Provide clear, professional coaching commentary and actionable concrete suggestions for improvement.

Return your response inside a valid JSON object matching this schema:
{
  "idealAnswer": "Direct and technically correct perfect model answer",
  "isMeaningful": true,
  "isRelevant": true,
  "isTechnicallyCorrect": true,
  "score": 0,
  "feedback": "Comprehensive professional coaching commentary and analysis",
  "improvements": "Actionable concrete suggestions for improvement"
}`;
    ...
    const evalResponse = await client.models.generateContent({
      ...
      config: {
        systemInstruction: "You are an elite technical lead and talent coach evaluating candidates with high precision and fairness.",
        responseMimeType: "application/json",
        responseSchema: {
          type: Type.OBJECT,
          properties: {
            ...
            score: { type: Type.INTEGER, description: "Precise integer score out of 10 matching the strict grading rules" },
          ...
          required: ["idealAnswer", "isMeaningful", "isRelevant", "isTechnicallyCorrect", "score", "feedback", "improvements"]
        }
      }
    });
    ...
    let idealAnswer = evalResult.idealAnswer || "";

    // Double check for invalid/gibberish answers programmatically
    if (!isMeaningful || !isRelevant || score === 0) {
      score = 0;
      isMeaningful = false;
      isRelevant = false;
      isTechnicallyCorrect = false;
...
export function generateFallbackIdealAnswer(question: string): string {
  return getImportedFallbackAnswer(question);
}
