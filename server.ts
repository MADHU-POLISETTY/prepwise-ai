...
app.post("/api/generate-questions", async (req, res) => {
  const { category, role = "Software Engineer", difficulty = "Intermediate", customTopic = "" } = req.body;

  const client = getGeminiClient();
  if (!client) {
    // Generate intelligent simulation fallback
    return res.json(getSimulatedQuestions(category, role, difficulty, customTopic));
  }
...
Constraints:
- Tailor the questions strictly to the selected Category and Role.
...
- If category is Technical, focus on coding standards, system design, engineering principles, data structures, algorithm details, or technical choices.
- If category is Aptitude, focus on problem solving, logical puzzles, quantitative thinking or system estimation.`;
    ...
    res.json(getSimulatedQuestions(category, role, difficulty, customTopic));
  }
});
...
2. Communication Score (0-100) - Evaluating articulation, coherence, grammar, structure, and brevity.
3. Technical Score (0-100) - Evaluating terminology accuracy, context depth, conceptual correctness, logic.
4. Confidence Score (0-100) - Evaluating assertive phrasing, lack of hesitation indicators, active verbs usage.
5. In-depth textual feedback in raw Markdown format. Write this in an elegant, inspiring, objective, and constructive style. Underline:
   - ### Core Strengths: (Detailed review of what the user nailed)
   - ### Technical & Structural Weaknesses: (Constructive criticism of missed marks, logic flaws, or superficial explanations)
   - ### Perfect Sample Answers: Provide 1-2 examples of high-performing model answers for the questions they struggled with.`;
    ...
    const response = await client.models.generateContent({
      ...
      contents: prompt,
      config: {
        systemInstruction: "You are a professional HR Director and Lead Engineering Executive evaluating an application candidate. Be fair, highly analytical, and provide clear educational pathways to success.",
        responseMimeType: "application/json",
        responseSchema: {
          type: Type.OBJECT,
          properties: {
            ...
            confidenceScore: { type: Type.INTEGER, description: "Assertiveness and clarity representation scoring (0-100)" },
            feedback: { type: Type.STRING, description: "Exhaustive personalized commentary and feedback in standard Markdown format" }
          },
          required: ["score", "communicationScore", "technicalScore", "confidenceScore", "feedback"]
        }
      }
    });
...
app.post("/api/analyze-resume", async (req, res) => {
  const { fileDataBase64, mimeType = "application/pdf", textContent = "" } = req.body;

  const client = getGeminiClient();
  if (!client) {
    return res.json(getSimulatedResumeAnalysis(textContent));
  }
  ...
  try {
    ...
    if (fileDataBase64) {
      // Clean base64 header if sent by standard FileReader
      ...
      contents.push({
        text: "Analyze the attached PDF resume. Identify the skills listed, extract professional strengths, outline areas for improvement, and deliver a concise executive summary of this applicant's competitive profile to jumpstart their custom interview coaching."
      });
    } else {
      contents.push({
        text: `Analyze the following raw text content of a resume:\n\n${textContent}\n\nIdentify skills listed, extract professional strengths, outline direct areas for improvement, and deliver a concise executive summary of this applicant's profile.`
      });
    }
    ...
    const response = await client.models.generateContent({
      ...
      contents: contents,
      config: {
        systemInstruction: "You are an elite talent acquisition researcher and resume auditor. Extract precision, factual data without adding unnecessary filler or praise.",
        responseMimeType: "application/json",
        responseSchema: {
          type: Type.OBJECT,
          properties: {
            ...
            improvements: {
              type: Type.ARRAY,
              items: { type: Type.STRING },
              description: "Clear actionable recommendations or missing skills to optimize the resume stand-out factor"
            },
            summary: {
              type: Type.STRING,
          ...
          required: ["skills", "strengths", "improvements", "summary"]
        }
      }
    });
    ...
    res.json(getSimulatedResumeAnalysis(textContent));
  }
});
...
function getSimulatedQuestions(category: string, role: string, difficulty: string, customTopic: string) {
  const genericTopic = customTopic || "Industry Standard Concepts";
  const searchStr = `${role} ${customTopic}`.toLowerCase();
  ...
  if (category === "HR") {
    if (isDevOps || isAWS || isGCP) {
      return [
        { id: 1, text: `Describe a situation where a critical production outage occurred due to a deployment or cloud infrastructure failure. How did you coordinate the post-mortem analysis and implement safeguards to prevent reoccurrences?` },
        { id: 2, text: `How do you handle disagreement with software developers regarding security controls or automated testing gates during an urgent hotfix release?` },
    ...
    return [
      { id: 1, text: `Tell me about a time you encountered a significant conflict while working on a ${genericTopic} task. How did you resolve it?` },
      { id: 2, text: `What is your approach to handling tight project deadlines, particularly when managing requirements for ${role}?` },
    ...
    return [
      { id: 1, text: `Explain the core conceptual differences between SQL and NoSQL storage paradigms, and when to use them for ${role} application data.` },
      { id: 2, text: `How do you secure server-side REST API endpoints from potential CSRF, XSS, or unauthorized header injection attacks?` },
...
function getSimulatedEvaluation(category: string, role: string, answers: any[]) {
  // Compute overall average based on whether answers are long/well written
  let scoreBase = 72;
  ...
  const finalScore = Math.round((communication + technical + confidence) / 3);
...
function getSimulatedResumeAnalysis(text: string) {
